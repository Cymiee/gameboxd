-- Marks a log's hours as manually set (e.g. cross-platform play), so a Steam
-- re-sync won't overwrite them. The import merge now skips manual logs.
alter table public.game_logs
  add column if not exists playtime_manual boolean not null default false;
